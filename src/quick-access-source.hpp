...
class QuickAccessSourceDelegate : public QStyledItemDelegate {
	Q_OBJECT
public:
	QuickAccessSourceDelegate(QObject* parent = nullptr, QuickAccessDock *dock = nullptr);
...
public:
	explicit QuickAccessSourceModel(QObject* parent = nullptr, SearchType searchType = SearchType::None);
	inline void setSources(std::vector<QuickAccessSource*> *newData) { _data = newData; }
...
public:
	...
	inline void addParent(QuickAccessSource* parent) { _parents.push_back(parent); }
	inline void addChild(QuickAccessSource* child) { _children.push_back(child); }
