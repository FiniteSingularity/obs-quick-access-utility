...
QuickAccessSourceDelegate::QuickAccessSourceDelegate(QObject* parent, QuickAccessDock* dock) :
	QStyledItemDelegate(parent), _dock(dock)
{
	...
	const QModelIndex& index) const
{
	...
	if (_dock->ShowFilters()) {
		...
		filterIcon.addFile(filtersIconPath.c_str(), QSize(), QIcon::Normal, QIcon::Off);
		filtersButton.icon = filterIcon;
		filtersButton.iconSize = QSize(16, 16);
		filtersButton.state = _filtersState | QStyle::State_Enabled;
		style->drawControl(QStyle::CE_PushButtonLabel, &filtersButton, painter, widget);
		loc += inc;
	}
	...
	if (_dock->ShowProperties() && item->hasProperties()) {
		...
		propsIcon.addFile(propertiesIconPath.c_str(), QSize(), QIcon::Normal, QIcon::Off);
		propertiesButton.icon = propsIcon;
		propertiesButton.iconSize = QSize(16, 16);
		propertiesButton.state = _propertiesState | QStyle::State_Enabled;
		style->drawControl(QStyle::CE_PushButtonLabel, &propertiesButton, painter, widget);
		loc += inc;
	}
...
QSize QuickAccessSourceDelegate::sizeHint(const QStyleOptionViewItem& /*option*/,
	const QModelIndex&/*index*/) const
{
	...
	const QModelIndex& index)
{
	...
	if (_dock->ClickableScenes() && event->type() == QEvent::MouseButtonRelease) {
...
QuickAccessSourceModel::QuickAccessSourceModel(QObject* parent, SearchType searchType)
	: QAbstractListModel(parent), _searchType(searchType)
{
...
Qt::ItemFlags QuickAccessSourceModel::flags(const QModelIndex& index) const
{
	...
	if (index.isValid())
		return Qt::ItemIsSelectable | defaultFlags; //return Qt::ItemIsSelectable | Qt::ItemIsDragEnabled | defaultFlags;
	else
		return Qt::ItemIsSelectable | defaultFlags; //Qt::ItemIsDropEnabled | Qt::ItemIsDragEnabled | defaultFlags;
}

bool QuickAccessSourceModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
...
bool QuickAccessSourceModel::insertRows(int row, int count, const QModelIndex& parent)
{
	if (parent.isValid())
		return false;
...
bool QuickAccessSourceModel::removeRows(int row, int count, const QModelIndex& parent)
{
	if (parent.isValid())
		return false;
...
QuickAccessSource::QuickAccessSource(obs_source_t* source)
{
	...
	_sourceClass = obs_source_is_group(source) ? SourceClass::Group : obs_source_is_scene(source) ? SourceClass::Scene : SourceClass::Source;
	BuildSearchTerms();
}
...
std::vector<SearchType> QuickAccessSource::search(std::string searchTerm)
{
	...
	for (auto& st : SearchTypes) {
		for (auto& haystack : _searchTerms[st]) {
			auto it = std::search(haystack.begin(), haystack.end(), searchTerm.begin(),
				searchTerm.end(), [](char a, char b) {
...
void QuickAccessSource::BuildSearchTerms()
{
	...
	_searchTerms[SearchType::Type].push_back(obs_source_get_display_name(source_id));
	...
	for (auto filter : filters) {
		...
		_searchTerms[SearchType::Filters].push_back(obs_source_get_display_name(filter_id));
		_searchTerms[SearchType::Filters].push_back(obs_source_get_name(filter));
	}
	...
	if (strcmp(source_id, MEDIA_SOURCE_ID) == 0) {
		...
		if (localFile) {
			...
			_searchTerms[SearchType::Url].push_back(obs_data_get_string(data, "input"));
		}
	}
...
void QuickAccessSource::removeParent(QuickAccessSource * parent)
{
	...
	if (auto it = std::find(_parents.begin(), _parents.end(), parent); it != _parents.end()) {
		_parents.erase(it);
	}
}
